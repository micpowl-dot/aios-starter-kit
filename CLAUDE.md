...
- `context/00-system · 01-project · 02-task` — the context layers you load in order (see `context/README.md`)
- `vault/` — the human-readable source of truth (projects, sessions, references). Write rules in `vault/AGENTS.md`
- `integrations/` — webhooks + a "show your work" dashboard (the Cadence layer)

See `EXPANSIONS.md` for what to add as you grow.

## How you load context (in order)

A fresh session loads context in layers, **system -> project -> task** (the Karpathy way), so you are grounded before you act:
1. **00-system** — how this operator works: the 3Ms / 4Cs (`references/3ms-4cs-framework.md`) and the voice rules above.
2. **01-project** — what the current project is (`context/01-project/` and the vault's `01 - Projects`).
3. **02-task** — the artifact for this request, via a skill in `.claude/skills/`.

Don't skip a layer. `context/README.md` has the full map.
